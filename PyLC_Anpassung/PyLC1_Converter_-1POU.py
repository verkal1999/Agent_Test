...
def list_pous(xml_path: str) -> List[str]:
    """
    Liefert alle POU-Namen aus einer TwinCAT PLCopen-XML-Datei zurück.
    """
    ...
    names: List[str] = []
    for pou in root.findall(".//plcopen:pou", ns):
        name = pou.get("name")
        if name:
            names.append(name)
    return names

def parse_pou_blocks(xml_path: str,
                     output_path: str = "generated_code_0.py",
                     target_pou_name: str | None = None) -> None:
    """
    ...
    - target_pou_name = None:
        Es wird – wie bisher – der letzte <pou>-Knoten aus der XML verwendet.
    - target_pou_name = "NameDesPOU":
        Es wird gezielt dieser POU ausgewertet.

    Die Hilfsdatei enthält:
        POU = {
            'pou_name': ...,
            'pou_type': ...,
            'input_vars': [... "Name:Typ" ...],
    ...
    if target_pou_name is not None:
        ...
        # Abwärtskompatibilität: letzten POU verwenden
    ...
    # Input- und Output-IDs (Mapping localId -> Ausdruck)
    ...
    # Blöcke dieses POU (Funktionsbausteine etc.)
...
if __name__ == "__main__":
    # Beispielaufruf:
    # parse_pou_blocks("export.xml", "generated_code_0.py", target_pou_name="HRL_OperatingModes")
    import sys

    xml = sys.argv[1] if len(sys.argv) > 1 else "export.xml"
    parse_pou_blocks(xml_path=xml, output_path="generated_code_0.py")
